"use client";
import { FixturaVideoContainer } from "@/components/containers/containers";
import {
  createStyles,
  Title,
  Text,
...
export function HeroText({ params }) {
...
export const HeroTitle = (props) => {
  const { classes } = useStyles();
  return <Title className={classes.title}>{props.children}</Title>;
};
export const HeroDateRange = (props) => {
  const { classes } = useStyles();
  return (
    <Center>
...
export const HeroControls = (props) => {
  const { classes } = useStyles();
